# CLI 入口(M7-T2):python -m html_to_docx --input <task.json>
...
def main() -> int:
    _force_utf8_stdio()
    parser = argparse.ArgumentParser(prog='html_to_docx', description='HTML 转 .docx 转换器(M7)')
    ...
    try:
        ...
        return 1
    except Exception as e:  # 未预期异常:兜底成契约形态,并把堆栈放进 detail 便于诊断(F9)
        _emit({'ok': False, 'error': {
            'kind': 'internal',
            'message': f'转换器内部错误: {e}',
